...
export type EchoKind = 'echo' | 'prime';
...
export type ZoneKind = 'weave' | 'pack' | 'breathe';
...
export type SegmentKind = 'launch' | 'weave' | 'pack' | 'pocket' | 'mix' | 'approach';
 ...
 * Same seed → same covers, pockets, and Echo homes.
 * Pattern matches Swarm's planner; contents are Anomaly-only.
 */
export function generateAnomalyLayout(rng: Rng): AnomalyLayout {
  ...
  for (const segment of segments) {
    ...
    if (segment.kind === 'pocket') {
      addPocket(rng, segment, covers, echoes, zones, density);
    } else if (segment.kind === 'pack') {
      addPack(rng, segment, echoes, zones, density);
    } else if (segment.kind === 'weave') {
      addWeave(rng, segment, echoes, zones, density);
    } else if (segment.kind === 'mix') {
      addMix(rng, segment, echoes, zones, density);
    } else if (segment.kind === 'launch') {
      // Quiet runway — feel the invert before the first Echo.
    } else {
      addApproach(rng, segment, echoes, zones, density);
    }
  }
  ...
  for (const spec of echoes) {
    if (hitsCover(spec.x, spec.y, layout.covers, 22)) {
      continue;
    }
    if (spec.x < AnomalyTuning.minHomeX || spec.x > world.width - 160) {
      continue;
    }
    layout.echoes.push(spec);
  }
...
function planSegments(rng: Rng, worldWidth: number): SpineSegment[] {
  ...
  for (let i = 0; i < n; i += 1) {
    laneY = clamp(laneY + rng.int(-80, 80), 200, 530);
    ...
    segments.push({
      ...
      x1: x0 + span,
      laneY,
      gapHalf: kind === 'pack' ? rng.int(50, 66) : rng.int(62, 88),
    });
  }
...
function pickBodyKinds(rng: Rng, n: number): SegmentKind[] {
  const template: SegmentKind[] = ['weave', 'pack', 'pocket', 'weave', 'pack', 'mix', 'pack'];
  const kinds = template.slice(0, n);
  while (kinds.length < n) {
    kinds.push(rng.chance(0.55) ? 'pack' : 'weave');
  }
  for (let i = 1; i < kinds.length - 1; i += 1) {
    ...
    if ((cur === 'pocket' && next === 'pocket') || (cur === 'pack' && next === 'weave')) {
      kinds[i] = next;
      kinds[i + 1] = cur;
    }
  }
...
function addLaneCovers(rng: Rng, segment: SpineSegment, covers: CoverSpec[]): void {
  ...
  for (let i = 0; i < pillars; i += 1) {
    ...
    covers.push({ x, y: 16 + topH / 2, w: rng.int(26, 40), h: topH });
    covers.push({ x: x + rng.int(-10, 10), y: bottomY0 + bottomH / 2, w: rng.int(26, 42), h: bottomH });
  }
}
...
function addPocket(
  ...
  zones.push({ x: left + 16, y: top + 14, w: w - 32, h: h - 28, kind: 'breathe' });
  zones.push({
    x: segment.x0 + 24,
    y: segment.laneY - 48,
  ...
  const roamers = Math.max(2, Math.round(4 * density));
...
function addWeave(
  ...
  zones.push({
    x: segment.x0 + 16,
    y: segment.laneY - segment.gapHalf - 8,
    w: Math.max(80, segment.x1 - segment.x0 - 32),
    h: segment.gapHalf * 2 + 16,
    kind: 'weave',
  });
  ...
  for (let i = 0; i < count; i += 1) {
    echoes.push({
      x: rng.float(segment.x0 + 40, segment.x1 - 40),
      y: segment.laneY + rng.float(-segment.gapHalf + 4, segment.gapHalf - 4),
      kind: 'echo',
      homeRadius: 36,
    });
  }
...
function addApproach(
  ...
  echoes.push({
    x: rng.float(segment.x0 + 80, segment.x1 - 180),
    y: segment.laneY + rng.float(-20, 20),
    kind: 'prime',
    homeRadius: 52,
  });
