...
export class HeatSink {
  ...
  constructor(capacity = AnomalyTuning.heatCapacity) {
  ...
  add(cost: number): void {
    ...
    this.coolDelayMs = AnomalyTuning.heatCoolDelayMs;
  ...
  update(deltaMs: number): void {
    ...
    this.current = Math.max(0, this.current - (AnomalyTuning.heatCoolPerSec * deltaMs) / 1000);
    if (this.overheated && this.current <= AnomalyTuning.heatRecoverAt) {
      this.overheated = false;
    }
  }
...
export class Mag {
  ...
  constructor(capacity = AnomalyTuning.ammoCapacity) {
    this.capacity = capacity;
...
/** Heat + ammo gate sustained fire. Facing comes from the caller (inverted thrust). */
export class AnomalyWeapon {
  ...
  tryFire(now: number): FireResult {
    ...
    this.heat.add(AnomalyTuning.heatPerShot);
    this.nextShotAt = now + AnomalyTuning.fireIntervalMs;
    return 'fired';
  }
}
...
export function spawnBolt(
  ...
  bolt.setVelocity(Math.cos(facing) * AnomalyTuning.boltSpeed, Math.sin(facing) * AnomalyTuning.boltSpeed);
  bolt.setBounce(0);
...
export function boltExpired(bolt: Phaser.Physics.Arcade.Image, now: number, worldWidth: number): boolean {
  ...
  if (now - born > AnomalyTuning.boltLifeMs) {
    return true;
  }
  return bolt.x < -40 || bolt.x > worldWidth + 40 || bolt.y < -40 || bolt.y > AnomalyTuning.world.height + 40;
}
